...
bool MifareKdfAn10922(uint8_t *key, uint8_t CryptoType,
    ...
    if (CryptoType == CRYPTO_TYPE_AES128) {
        ...
        // TODO: Can we save some space on the stack by reusing 
        //       SessionIV from DESFireCrypto.h ???
...
/* TODO / NOTE: The current firmware compiled with ``make clean && make desfire`` crashes out with 
 *              the error: "Application Section size 66205 excedes maximum allowed 65536. 
 *              Please disable some features in Makefile" -- even with extended AES-192/256 
 *              support disabled. Can we save some space by omitting these large buffers and 
 *              compute these values on the fly (possibly with a penalty of slower scramble 
 *              function performance ???
 */
const uint8_t scrambleLutBuffer[256] = {
...
void scramble(uint8_t *arr, uint8_t len) {
    const uint8_t *lut = &scrambleLutBuffer[0];
    for (int i = 0; i < len;  i++) {
        arr[i] = lut[arr[i]];
    }
}
