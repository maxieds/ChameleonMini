 ...
 * 
 * "Only the first some of these constants are actually used – up to rcon[10] for 
 *  AES-128 (as 11 round keys are needed), 
...
void SetupLocalAESContext(AES_ctx *ctx, uint8_t cryptoType) {
    if (cryptoType == _CRYPTO_TYPE_AES192) {
        ...
        ctx->KeySize = 24;
        ctx->KeyExpSize = 208;
    } else if (cryptoType == _CRYPTO_TYPE_AES256) {
        ctx->Nk = 8;
        ctx->Nr = 14;
        ctx->KeySize = 32;
        ctx->KeyExpSize = 240;
    } else { // Defaults to AES-128:
        ctx->Nk = 4;
        ctx->Nr = 10;
        ctx->KeySize = 16;
        ctx->KeyExpSize = 176;
